...
from fastapi_django.mail.utils import DNS_NAME
from fastapi_django.utils.encoding import force_bytes, force_str, punycode
...
class EmailMessage:
    ...
    def __init__(
        ...
        reply_to=None,
        connection=None,
        provider = settings.DEFAULT_EMAIL_PROVIDER_ALIAS,
    ):
        """
    ...
    def get_connection(self, fail_silently=False):
        from fastapi_django.mail import get_connection

        if not self.connection:
            self.connection = get_connection(fail_silently=fail_silently, provider=self.provider)
    ...
    def attach_file(self, path, mimetype=None):
        ...
        """
        path = Path(path)
        with path.open("rb") as file:  # TODO: асинхронно?
            content = file.read()
